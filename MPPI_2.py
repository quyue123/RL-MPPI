...
class TOLD(nn.Module):
    ...
    def plan(self, env, obs):
        ...
        pi_actions = torch.empty(horizon, action_dim)
        obs_pi = obs
        for t in range(horizon): 
            _, pi_actions[t,:] = agent.get_action(obs_pi)    
            obs_pi,r,_,_ = agent_model.step(obs_pi,pi_actions[t,:])
...
dataFrame_MPPI2_try = pd.DataFrame(zip(Method, Return, Samples, Time, Std, Horizon, Iteration),\
...
print(dataFrame_MPPI2_try)
dataFrame_MPPI2_try.to_csv("/root/quyue/data/dataFrame_MPPI2_try.csv",index=False,sep=',')
# return 曲线 
