...
#     def push(self, data):
#         if len(self.storage) == self.buffer_maxlen:
#             self.storage[int(self.ptr)] = data[0:-1]
#             #self.priority_buffer[int(self.ptr)] = data[-1]
#             self.ptr = (self.ptr + 1) % self.buffer_maxlen
#         else:
...
class DSAC:
    ...
    def get_q(self,state):    # 也可以在 update 模块直接输出
        state = torch.FloatTensor(state)
        action1, _, _ = self.policy_net.evaluate(state)
        #x = torch.cat([state, action1], 1)
        q, _, _ = self.q1_net.evaluate(state, action1)
        return q
    ...
    def load(self):
        self.policy_net.load_state_dict(torch.load(args.file_name + '_actor.pth'))
        self.q1_net.load_state_dict(torch.load(args.file_name + '_critic.pth'))
        # print("====================================")
...
def main(env,  agent, Episode, batch_size):
    if args.mode == 'train':
        ...
        for episode in range(Episode):
            ...
            writer.add_scalar('Return in each episode(train)', score, episode)
